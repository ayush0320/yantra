...
export const addBlog = async (req, res) => {
  try {
    const isMultipart = !!req.body.blog;
    const blogData = isMultipart ? JSON.parse(req.body.blog) : req.body;

    const { title, subtitle, description, category, isPublished, image } =
      blogData;
    const imageFile = req.file;

    if (!title || !description || !category || (!imageFile && !image)) {
      return res
        .status(400)
        .json({ success: false, message: "All fields are required!" });
    }
...
export const generateContent = async (req, res) => {
  try {
    ...
    const content = await main(
      prompt + "Generate a blog content for this topic in simple language.",
    );
